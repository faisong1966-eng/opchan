...
app.post("/open-lootbox", async (req, res) => {
  ...
  // รีเซ็ตสถานะเรตพิเศษหรือลดจำนวนเกลือที่ใช้ไปจริงลงใน Database
  let finalForceRateType = forceRateType;
  let finalSaltCount = currentSaltCount;
  
  if (forceRateType.startsWith('always_')) {
      finalForceRateType = 'normal'; // ถ้าเป็นการล็อกรางวัลแบบครั้งเดียวหรือล็อกถาวรแล้วแต่ต้องการให้รีเซ็ต สามารถปรับตรงนี้ได้ (ตอนนี้เซ็ตให้ปลดล็อกกลับเป็น normal หลังสุ่มเสร็จ)
  }
